/*
This is just an example for a Windows form GUI (by C#) , please look on GIT for the other example with the implemented Windows form, if
 * you want to the application with the windows form. 
...
namespace WTXModbus
{
    ...
    /// Alternative: The timer could also be implemented in class "Modbus_TCP".
    /// Futhermore the data is only displayed, if the values have changed to save reconstruction time on the GUI Form. 
    /// 
    /// Beside a form the GUI could also be a console application by applying that in program.cs instead of a windows form. 
    /// Therefore the design of the classes and its relations are seperated in 
    /// connection specific classes and interface (class "Modbus_TCP", interface "Communication_Device_Interface", "Communication_Interface")
     ...
     partial class GUI : Form
    {
        ...
        public void set_GUI_rows()
        {
            ...
            if (WTX_obj.application_mode == 0) // case 1) Standard application. Initializing the description and a placeholder for the values in the data grid.
            {
                ...
                if (WTX_obj.application_mode == 2) // case 2) Filler application. Initializing the description and a placeholder for the values in the data grid.
            {
        ...
        public void Read_DataReceived(IDeviceValues Device_Values)
        {
            ...
            // Second if-condition: Only if the length of the previous data array is larger than 14, it will be possible to access values otherwise you get an "ArrayoutofBounds"-Exception 
            {
                ...
                {
                    if (Device_Values.application_mode == 0)  // Checks whether the WTX device is in standard application mode (application=0) or in filler application mode (application=2). 
                        this.is_standard = true;

                    else
                     if (Device_Values.application_mode == 2)
        ...
        public void reset_values()
        {
            ...
            //foreach (DataGridViewTextBoxColumn c in dataGridView1.Columns)c.Width = 120;
            ...
            if (this.is_standard == true)             // In the standard application: 
            {
                for (int index = 27; index <= 35; index++)
                    dataGridView1.Rows[(index + 1)].Cells[7].Value = data_str_arr[index];
            }
            else
                if (this.is_standard == false)        // In the filler application: 
            {   
