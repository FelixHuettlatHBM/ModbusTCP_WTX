...
namespace Hbm.Devices.WTXModbus
{
    ...
    interface ICallAsyncEvent
    {
        ...
        void Async_Call(/*ushort wordNumberParam, */ushort commandParam, Action<IDeviceValues> callbackParam);

        void ReadDoWork(object sender, DoWorkEventArgs e);

        //IDevice_Values read_data(BackgroundWorker worker);

        void ReadCompleted(object sender, RunWorkerCompletedEventArgs e);

        void WriteDoWork(object sender, DoWorkEventArgs e);

        void WriteCompleted(object sender, RunWorkerCompletedEventArgs e);
    }
}
